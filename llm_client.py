...
class LLMClient:
    ...
    def generate_reviews(
        ...
        """
        user_prompt = build_review_prompt(product_name, sku, ratings)
        ...
        for attempt in range(1, self.config.max_retries + 1):
            try:
                ...
                # Exponential backoff on rate-limits or server errors
                ...
                if raw_reviews and len(raw_reviews) > 0:
                    ...
                    for idx, r in enumerate(raw_reviews[:len(ratings)]):
                        ...
                        r_rating = int(r.get("rating", ratings[idx] if idx < len(ratings) else 5))
